...
fn filepath_to_string(fp: FilePath) -> String {
    match fp {
        FilePath::Url(url) => url.to_file_path().unwrap_or_default().to_string_lossy().to_string(),
        FilePath::Path(path) => path.to_string_lossy().to_string(),
    }
}

#[tauri::command]
pub async fn pick_directory(app: AppHandle, prompt: Option<String>) -> Result<Option<String>, String> {
    let chosen: Option<FilePath> = app.dialog().file()
